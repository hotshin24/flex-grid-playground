 ...
 * main.js — 부품 조립
 *
 * store · renderer · controls · router 를 이어 붙인다. 이 파일은 배선만 하고
 * 속성을 알지 못한다. 컨트롤 목록은 전량 스키마에서 나온다.
 *
 * 여기가 맡는 것 — 토픽 · 탭 · 프리셋 · 뷰 설정 · 아이템 조작 · 코드 출력 ·
 * 되돌리기 버튼 · 측정 통지. 탭 안쪽 내용은 각 ui 모듈이 자기 몫을 짓는다.
 *
 * 아이템 버튼의 이벤트는 여기서 위임으로 건다. 규칙 3 대로 각 모듈이 자기
 * 뿌리에 하나씩 거는 방식이고, 모으는 중앙 파일은 두지 않는다.
 */
...
/** 토픽에서 주입받는다. 토픽이 늘면 자기 목록을 여기에 더한다. */
const EXPLAIN = {
  flex: {
    schema: FLEX_SCHEMA,
