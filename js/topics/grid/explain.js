...
export const GRID_EXPLAIN_SAMPLES = {
   ...
   * 음수 라인을 빼고 양수 셋을 둔다.
   *
   * 음수 가로 라인은 명시한 행을 기준으로 센다. 이 판은 행을 선언하지 않아
   * 모두 암시 행이고, 그래서 -2 가 auto 와 같은 자리를 가리켰다. 음수를 제대로
   * 보이려면 행을 선언한 다른 판이 필요한데, 음수 라인 자체는 grid-column-start
   * 에서 이미 다루므로 여기서는 행 번호가 옮겨 가는 것만 보인다.
   */
  'grid-row-start': [
    { val: 'auto', desc: '자동 배치에 맡깁니다 (기본값)' },
    { val: '2', desc: '2번 가로 라인에서 시작. 둘째 줄로 내려갑니다' },
    { val: '3', desc: '3번 가로 라인. 없는 행이면 자동으로 만들어집니다' },
  ],
  'grid-row-end': [
    { val: 'auto', desc: '한 칸만 차지 (기본값)' },
