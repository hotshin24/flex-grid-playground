...
v0.1 파일 넷은 M7 파일 정리(PRD 7.2)에서 지웠다 — `index.html`(절차 4) ·
`js/app.js` · `js/data.js` · `css/style.css`(절차 5). 작업 트리에 남은 v0.1
코드는 없고, 절대 규칙에도 예외가 없다.

원본은 **`v0.1-archive` 태그**(`ae4aa07`)에 통째로 있다. 로컬과 원격 양쪽에
있고, 그 커밋의 트리는 v0.1 파일만 담고 있다.

```bash
git show v0.1-archive:js/data.js      # 원본 읽기
git ls-tree -r --name-only v0.1-archive
```

**이 태그는 기념물이 아니라 게이트가 읽는 자료다.** 지우거나 다른 커밋으로
옮기면 안 된다.

| 게이트 | 태그에서 무엇을 대조하는가 |
|---|---|
| `check-examples.mjs` | 예제 18건의 제목·설명·CSS·HTML 을 글자 단위로 |
| `check-challenges.mjs` | 이관 챌린지 7건의 제목·설명·힌트, 아이템 색 22개가 원본 팔레트와 같은지 |

태그를 찾지 못하면 두 게이트는 그 절만 건너뛴다. 실패로 처리하지 않는다 —
얕은 클론처럼 바깥 사정으로 게이트가 무너지면 안 되기 때문이다. 대신
**건너뛴 채로 통과한 것을 완전한 통과로 보고하지 않는다.**

이관 결과는 이랬다. 대조가 무엇을 덮는지 알아 두려면 이 숫자가 기준이다.

| 콘텐츠 | 결과 |
|---|---|
| EXAMPLES | 18 → 18. 제목·설명·CSS·HTML 불일치 0 |
| CHALLENGES | 8 → 이관 7 (target 7/7 일치) + 재작성 1 (「사이드바 레이아웃」) |
| EXPLAIN_DATA | 12속성 52값 → 값 슬롯 52/52, 누락 0 |

설명 문장은 그대로 옮긴 것이 아니라 다시 쓴 자리가 많다 — 값 설명 34개 중
29개, 속성 desc·tip 24문장 중 19개다. 값이 빠지지 않는 한 다시 쓰는 것을
허용했고, 실제로 `align-items: stretch` 의 "아이템에 크기가 없을 때만 동작",
`flex: auto` 의 "= 1 1 auto", `order` 의 접근성 경고처럼 v0.1 에 없던 정보가
늘었다. **앞으로 설명 문장을 고칠 때도 같은 선을 지킨다 — 값 하나, 단서
하나가 사라지면 그건 개선이 아니라 손실이다.**

## 진입점

진입점은 `index.html` 하나다. PRD 7.2 절차 **1~5가 전부 끝났다** —
회귀 검증(1) · `v0.1-archive` 태그(2) · `js/data.js` 이관 확인(3) ·
진입점 교체(4) · v0.1 파일 삭제(5). `index-v1.html`이 4번에서 `index.html`
이름을 받았고, 남은 v0.1 파일은 없다.
...
**예외 없이 전 파일에 적용된다.** 집계에서 빼는 파일은 하나도 없다.

M6까지는 v0.1 파일 넷을 참조 전용으로 두고 위반 집계에서 뺐다. M7 파일 정리로
그 파일들이 사라져 예외 자체가 없어졌다. 감사 보고에 "참조(비적용)" 칸을 두지
않는다 — 지금 리포지터리의 모든 코드가 이 규칙을 지켜야 한다.
